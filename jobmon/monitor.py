...
def get_jobs_in_queue( max_name_length ):
    ...
    for i in range( 1, len( lines ) ):
        ...
        job_in_queue[ job_name.decode( ).strip( ) ] = job_status.decode() 
    return job_in_queue
...
def create_new_run_vasp( run ):
    folder = 'RUN' + run
    os.system( 'cp -a ' + folder + '/CONTCAR POSCAR' )

def create_new_run_qe( run ):
    # require make.py that take `run` as argument
    folder = 'RUN' + run
    os.system( 'python make.py ' + run )
...
def resubmit( jobs, jobs_in_queue, time_from_last_write, submit_cmd, send_email = False, email_setting = {}, 
              if_job_done = if_vasp_done, output = 'OUTCAR', max_name_length = 20,  
              backup_files = [ 'POSCAR', 'OUTCAR', 'INCAR', 'KPOINTS', 'vasprun.xml', 'CONTCAR' ],
    ...
    for ID, name in enumerate( sorted( jobs.keys( ) ) ):
        ...
        if jobs[ name ][ 'status' ] == "FINISHED":
            ...
            job_done, niter, nmaxiters, mtime = if_job_done( output )
            if not job_done:
                print ( output + 'is not done' )
            else:
                print ( output + 'is done' )
            print ( 'Number of iterations %i' %( niter ) )
            print ( output + 'was last updated %i s ago' %( -mtime ) )
            if name not in jobs_in_queue.keys():
            ...
            if ( not job_done or niter == nmaxiters ) and ( mtime < -time_from_last_write ) and ( name not in jobs_in_queue.keys( ) ) and niter > 0:
                run =  get_run_num( ) + 1
                run = str( run )
                backup( run, backup_files = backup_files )
                create_new_run( run )
                print ( "%s" %( 'RESUBMITING... ') )
                sys.stdout.flush( )
    ...
    jobs_in_queue = get_jobs_in_queue( max_name_length )
...
def run_job_monitor( jobs, time_from_last_write = 600, time_to_check= 1500, submit_cmd = "sbatch job", send_email = False, email_setting = { },
                     monitorID = 'Automatic Monitoring', max_name_length = 20, 
    ...
    for ID, name in enumerate( sorted( jobs.keys( ) ) ):
        os.chdir( jobs[ name ][ 'dir' ] )
        if not os.path.isfile( output ) and name not in jobs_in_queue.keys( ) and jobs[ name ][ 'status' ] != 'FINISHED':
    ...
    while nfinished < ntotal:
        print ( time.ctime( ) )
        jobs_in_queue             = get_jobs_in_queue( max_name_length = max_name_length )
        jobs                      = resubmit( jobs, jobs_in_queue, time_from_last_write, submit_cmd, send_email, email_setting,
                                              max_name_length = max_name_length, if_job_done = if_job_done, output = output, 
                                              backup_files = backup_files, create_new_run = create_new_run )
        sys.stdout.flush()
        nfinished, ntotal, nerror = count_jobs( jobs )
        print ( " " )
